...
function setStepDotFallback(isFallback) {
  ...
  if (!dot) return;
  if (isFallback) {
    dot.classList.add('step-dot-fallback');
  } else {
    dot.classList.remove('step-dot-fallback');
  }
}
...
async function fetchPlantList() {
  ...
  try {
    ...
    if (res && res.ok) {
      // ✅ 백엔드 정상 응답 → 점은 기본(초록색) 유지
      const data = await res.json();
      results = data.results || data || [];
    } else {
      // ❗ 백엔드 에러 or 응답 없음 → plant/plant.json 폴백
